...
%                     'color','k','LineWidth', 1.5);
                 plot(mean(Xx(1,:)),box_mncor(wk),'o',...
                    'color','w','MarkerEdgeColor', 'w','MarkerFaceColor', 'w','MarkerSize',4);
            end
        end        
...
if confs.flipcolors==1    
    ...
    if ~isempty(confs.legend)
        h.lg = legend(h.sc(1,:),confs.legend,'location','best');
        h.lg.Position(2) = h.lg.Position(2) + confs.add_dis;
        h.lg.Position(1) = h.lg.Position(1) + 0.01;
    end
else
    
    % add a legend based on box colors
    if ~isempty(confs.legend)
        h.lg = legend(h.bx(1,:),confs.legend,'location','best');
        h.lg.Position(2) = h.lg.Position(2) + confs.add_dis;
        h.lg.Position(1) = h.lg.Position(1) + 0.01;
    end
end
